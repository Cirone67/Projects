...
public class Charge extends Objet{
   ...
   public Charge(int id, int noeud, double norme, double angle){
     ...
     public void draw(GraphicsContext gc, double largeur){
         ...
         double xmil = x2 + ar*(-(x2-x)/Math.abs(this.norme)); //pointe 
         double ymil = y2 + ar*(-(y2-y)/Math.abs(this.norme)); 
        
         double x1fleche = xmil + wid*((-(y2-y))/Math.abs(this.norme));
         double y1fleche = ymil + wid*((x2-x)/Math.abs(this.norme));
         
         double x2fleche = xmil - wid*((-(y2-y))/Math.abs(this.norme));
         double y2fleche = ymil - wid*((x2-x)/Math.abs(this.norme));
